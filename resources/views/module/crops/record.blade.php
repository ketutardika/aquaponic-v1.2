...
<nav class="page-breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">Record Data {{ $crops->crop_name }}</li>
  </ol>
</nav>
  ...
  <div class="col-md-12 grid-margin stretch-card">
    <div class="card">
      <div class="card-body">
        ...
        <h6 class="card-title">Add Record Data {{ $crops->crop_name }}</h6>

        <form class="forms-sample" action="{{ route('crops.record_store', $crops->id) }}" method="POST">
          @csrf
          <input type="hidden" name="crop_id" value="{{ old('crop_id', $crops->id) }}" class="form-control">
          ...
          <div class="mb-3">
            ...
            <input type="text" name="color" value="{{ old('color') }}" class="form-control @error('color') is-invalid @enderror" id="exampleInputUsername1" autocomplete="off" placeholder="Color">
            <!-- error message untuk title -->
            @error('color')
            <p class="mb-3">
                {{ $message }}
