...
<div class="d-flex justify-content-between align-items-center flex-wrap grid-margin">
  <div>
    <h4 class="mb-3 mb-md-0">{{ $crops->crop_name }}</h4>
  </div>
</div>
          ...
          <div class="card-body">
            ...
            <div class="row">
              <div class="col-6 col-md-12 col-xl-5">
                <h3 id="SensorValue-{{$sensordevice->sensor_devices_id}}" class="mb-2" style="display: inline-block;">0</h3><span id="SensorMeasure-{{$sensordevice->sensor_devices_id}}"> C</span>
                <div class="d-flex align-items-baseline">
                  <p class="text-success">Last Update : 
                    <span id="SensorCheck-{{$sensordevice->sensor_devices_id}}"></span>
                  </p>
                </div>
              </div>
            </div>
...
@push('custom-scripts') 
  ...
  jQuery.ajax({
      type:'GET',
      url: "/data/{{$sensordevice->sensor_devices_id}}/live",
      data: { id: lastId }, //Add request data
      success: function (data) {
        $('#SensorValue-{{$sensordevice->sensor_devices_id}}').text(data.value);
        $('#SensorMeasure-{{$sensordevice->sensor_devices_id}}').text(' '+data.measurement);
        $('#SensorCheck-{{$sensordevice->sensor_devices_id}}').text(data.update);
      }
  });
  };
